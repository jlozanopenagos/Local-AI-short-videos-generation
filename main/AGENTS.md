...
| `main/tools/` | Dedicated CLI utilities organized into 4 functional subfolders: <ul><li>**`database/`**: `db.py` (DB & CSV manager), `sync_prompts.py` (queue sync), `assign_ids.py` (canonical ID assigner)</li><li>**`auditing/`**: `status_scraper.py` (pipeline status compiler), `json_health_checker.py` (JSON audit/repair), `check_language_mixing.py` (multilingual contamination check), `scrapper_script.py` (script review exporter)</li><li>**`modifiers/`**: `script_modifier.py` (verbatim single, mass & ready_scripts script repair), `voice_modifier.py` (voice audition & recasting), `image_modifier.py` (scene recreation & chalkboard editor), `subtitles_modifier.py` (subtitle timing & ASS editor)</li><li>**`prompt_builders/`**: `generate_editor_prompts_csv.py` (system editor prompts generator)</li></ul> |
...
15. **Google Sheets Connectivity & Safe Column Modification (`main/connectivity/`)**:
    - **Feature 1 (`sync_sheets.py`, legacy alias `cli.py`)**: Pulls sheet records into `D:\AI\output\connectivity\synced_sheets\_3_columns\` (Columns A-C) or `_4_columns\` (Columns A-D).
    ...
    - **Feature 3 (`reconcile_scripts.py`)**: Audits local scripts (`D:\AI\output\scripts_to_see`) against Google Sheets data (`synced_sheets/_3_columns`), preserves sheet order and canonical expression names, appends new local entries at end, and outputs mass-update-ready CSVs to `D:\AI\output\connectivity\scripts_to_post\`.
    - **Feature 4 (`post_scripts.py`)**: Pushes scripts from `scripts_to_post` (or `scripts_to_see` / `synced_sheets/_4_columns`) to Google Sheets. In standard mode (choice 1), affects **only Columns A, B, and C**; Column D (`SCRIPT_CHANGED`) is strictly preserved unless 4 columns mode (choice 2) is explicitly selected.
