...
15. **Google Sheets Connectivity & Safe Column Modification (`main/connectivity/`)**:
    - **Feature 1 (`cli.py`)**: Pulls sheet records (`ID`, `expression`, `script`) into `D:\AI\output\connectivity\<lang>\<type>\<lang>_<type>_connectivity.csv`.
    - **Feature 2 (`corrected_scripts_fetching.py`)**: Filters Column D (`SCRIPT_CHANGED`) and exports non-empty corrections directly into `main/input/csv/script_to_change/<lang>_<type>_script_to_change.csv` with schema `ID, NEW_SCRIPT`.
    - **Feature 3 (`post_scripts.py`)**: Pushes scripts from `D:\AI\output\scripts_to_see` back to Google Sheets.
    - **Safety Guarantee for Column D**: Updating Google Sheets via `post_scripts.py` affects **only Columns A, B, and C**. Column D (`SCRIPT_CHANGED`) is **never touched or overwritten**.
    - **Option A Dynamic Routing**: All 16 sheets route through a master Google Apps Script Web App URL with `?id=<SPREADSHEET_ID>`.
