...
| File / Folder | Role |
| :--- | :--- |
| `main.py` (root) | Main orchestrator to run pipeline stages sequentially with pre-flight health verification. |
| `config/` | Central configuration & health package: `config/settings.py` (paths, endpoints, voices, pauses) and `config/health.py` (centralized LLM & ComfyUI connectivity checks & assertions). |
